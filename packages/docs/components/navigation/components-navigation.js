...
const NavLink = ({ href, children }) => {
  ...
  return (
    <li id={href} to={href} ouiaId={href} className="pf-v6-c-nav__item">
      <Link href={href}>
        <a
          className={classnames('pf-v6-c-nav__link', {
...
const NavigationGroup = ({ group, items, packageName }) => {
  ...
  return (
    <section key={group} title={group} className={classnames('pf-v6-c-nav__section', classes.capitalize)}>
      <h2 className="pf-v6-c-nav__section-title">{group}</h2>
      <ul className={classnames('pf-v6-c-nav__list', classes.capitalize)}>
        {items.map((item) => {
          const title = typeof item === 'object' ? item.title : item;
          const name = typeof item === 'object' ? item.name : item;
          return (
...
const ExpandableLink = ({ packageName, groups }) => {
  ...
  return (
    <li
      className={classnames('pf-v6-c-nav__item', 'pf-m-expandable', {
    ...
    >
      <button className="pf-v6-c-nav__link" onClick={() => setIsExpandend((prev) => !prev)}>
...
const Navigation = () => {
  return (
    <nav className="pf-v6-c-nav" ouiaId="docs-nav">
