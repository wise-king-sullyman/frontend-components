...
const SortBy: React.FunctionComponent<SortByProps> = ({ direction = SortByDirection.asc, onSortChange = () => undefined }) => (
  <Button icon={<Icon size="md">{direction === SortByDirection.asc ? <SortAmountUpIcon /> : <SortAmountDownIcon />}</Icon>} variant="plain" onClick={(e) => onSortChange(e, flipDirection(direction))} />
);
